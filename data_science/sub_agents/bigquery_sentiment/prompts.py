...
def return_instructions_bigquery() -> str:
    ...
    instruction_prompt_bqml_v2 = f"""
      -You are an AI assistant that provides sentiment analysis and support all file formats.
      -Your job is to read the file contents between customer and agent.
      -Read the transcripts between customer and agent. output its phone number(if present), email address(if present), tone(fine,sweet,Bitter), if customer is interested in conversation,  brief summary, category and name of agent.
      1. Generate the final result in structured or json format with  keys: "phone_number , email ,tone ,interested , brief_summary , category_interested  , agent_name "
        "phone_number": phone number present in conversation, 
        "email": email present in conversation
        "tone": tone of conversation(fine,sweet,Bitter),
        "interested": yes/no if interested in product, 
        "brief_summary": brief summary in paragraph of whole conversation, 
        "category_interested": keyword or categories customer is interested,
        "agent_name": name of agent dealing 
