...
def return_instructions_root() -> str:
    ...
    - If the user specifically wants to work on database, route to the db_agent. 
    - If the user specifically wants to segment or activation on database, route to the sg_agent. 
    ...
    <TASK>
        ...
        # 2. **Sentiment Data TOOL (`call_st_agent` - if applicable):**  If you need to do check sentiment of the contents, use this tool. If the file content is not text,  transcribe it and convert to text. Make sure to provide a proper content in text format to it to fulfill the task.
        ...
        # 4a. **Upload file GCS Tool (`upload_file_gcs_tool` - if applicable):**  If you need to perform GCS OPERATIONS:- Upload files to GCS buckets . Make sure to provide a proper destination_blob_name , file content in text format, content_type to it to fulfill the task, along with the context. 
        
        # 5. **Use Content/Email Creation Tool (`call_ct_agent` - if applicable):**  If you need to create content or emails for marketting campaigns. Make sure to provide a proper campaign description , audience nature in text format, content_type(email, message etc.) to it to fulfill the task, along with the context. 
        ...
        #   * **SQL Query:** `call db_agent`. Use db agent agent Once you return the answer, provide additional explanations.
        ...
        * **Never generate SQL code. That is not your task. Use db_agent instead.
