...
#credentials = service_account.Credentials.from_service_account_file('gen-lang-client-0490809986-05e0025fb41e.json')
MAX_NUM_ROWS = 50
...
def run_bq_gcs_activation(
    tool_context: ToolContext,
    sql_string: str,
    destination_blob_name: str,
    bucket_name: Optional[str] = get_env_var("BQ_BUCKET_ID")    ):
    try:
        destination_blob_name= destination_blob_name.replace(".csv","")
        sql_string=f"""  EXPORT
     DATA
     OPTIONS(uri='gs://{bucket_name}/{destination_blob_name}_*.csv', format='CSV',
             field_delimiter=',', header=true, overwrite=true)
     FROM
     ({sql_string});  -- Replace with your query
    """
        query_job = get_bq_client().query(sql_string)
        results = query_job.result()  # Get the query results
        return(f"successfull activation of file to gs://{bucket_name}/{destination_blob_name}_*.csv")    
    except (
        Exception
    ) as e:  # Catch generic exceptions from BigQuery  # pylint: disable=broad-exception-caught
